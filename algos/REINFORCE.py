...
def reinforce(env, episodes=1000, gamma=0.99, lr=1e-2):
    ...
    for ep in range(episodes):
        ...
        optimizer.step() # DO NOT UPDATE MULTIPLE STEPS IN A ROW WITHOUT INTERACTION WITH THE ENVIRONMENT, OTHERWISE THE POLICY GRADIENT UPDATE WILL NOT BE CORRECT!
...
def run_reinforce_lunarlander(env, model, render=False):
    ...
    while not done:
        ...
        action = torch.multinomial(model(state_tensor), num_samples=1).item()
        next_state, reward, done = env.step(action)
        total_reward += reward
        state = next_state
